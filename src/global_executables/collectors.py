...
def record(command, ecosystem, package, version=None, repository=None, source="fixture", confidence="direct", alias_of=None):
...
def npm_metadata(value, source="npm"):
    ...
    for p in packages:
        ...
        for command in bins:
            out.append(record(command,"npm",p["name"],p.get("version"),p.get("repository",{}).get("url") if isinstance(p.get("repository"),dict) else p.get("repository"),source))
    return out

def pypi_wheel(path: Path, package: str, version=None, repository=None):
    out=[]
...
def crates_manifest(text, package, version=None, repository=None, source="Cargo.toml"):
    ...
    return [record(n,"crates",package,version,repository,source) for n in names]
