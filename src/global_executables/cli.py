...
def main():
    ...
    build=sub.add_parser("build"); build.add_argument("inputs",nargs="+",type=Path); build.add_argument("--root",type=Path,default=Path.cwd()); build.add_argument("--snapshot")
    ...
    a=p.parse_args()
    if a.action=="build": rebuild(a.root,a.inputs,a.snapshot); return
